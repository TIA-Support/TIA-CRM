...
def summary():
    ...
    if scoped:
        company_q = company_q.filter(
            or_(Company.assigned_to == uid, Company.created_by == uid)
        )

        deal_q = deal_q.filter(
            or_(Deal.assigned_to == uid, Deal.created_by == uid)
        )
    ...
    # -------------------------
    # NEW PIPELINE ANALYTICS
    # -------------------------

    won_deals = deal_q.filter(
        Deal.stage == "won"
    ).count()

    lost_deals = deal_q.filter(
        Deal.stage == "lost"
    ).count()
    ...
    avg_deal_size = avg_query.scalar() or 0
    ...
    for deal in open_deals:
        weighted_forecast += (
            ...
            * probabilities.get(deal.stage, 0)
        )

    # -------------------------

    tasks_due_today = task_q.filter(
        Task.status == "pending",
        Task.due_date <= date.today()
    ).count()

    calls_today_q = Activity.query.filter(
        Activity.type == "call",
        func.date(Activity.occurred_at) == date.today()
    )

    if scoped:
        calls_today_q = calls_today_q.filter(
    ...
    followups_due_q = Activity.query.filter(
        ...
        Activity.next_follow_up <= date.today()
    )

    if scoped:
        followups_due_q = followups_due_q.filter(
            Activity.user_id == uid
        )
    ...
    leaderboard = (
        db.session.query(
            User.name,
            func.count(Activity.id).label("calls_made")
        )
        .join(Activity, Activity.user_id == User.id)
        .filter(
            Activity.type == "call",
            Activity.occurred_at >= date.today().replace(day=1)
        )
        .group_by(User.id)
